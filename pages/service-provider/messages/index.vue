      ...
      <div class="lg:flex">
        <div class="lg:w-3/4 lg:mr-3 shadow-md bg-white">
          <div class="chat-board" id="message-container">
            ...
            <div class="p-2 lg:p-5" v-if="selectedUserIndex>-1">
              ...
              <div class="py-4" v-for="(message ,index) in messageHistory" :key="`${index}`">
                ...
                <div v-bind:class="[message.from_me ? 'items-end' : '',message.from_me ? 'flex-col' : '', 'flex', 'mt-5']">
                  <div class="message-from-avatar" v-if="!message.from_me">
                    <img class="h-10 w-10 rounded-full" :src="selectedAvarta" />
                  </div>
                  <div v-bind:class="[message.from_me ? 'flex items-end flex-col mt-5' : 'ml-2 flex flex-col']">
                    <div v-bind:class="[message.from_me ? 'message--to' : 'message--from', 'messages']" v-for="(item, i) in message.messages" :key="`${i}`">{{item}}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="p-5 mt-5 border-t border-solid border-gray-300">
            <div class="flex">
              <div class="flex-grow">
                <input type="text" class="h-full w-full border-0 text-gray-600 outline-none pr-2" placeholder="Type Message" v-bind:value="chat_message" v-on:input="chat_message = $event.target.value" v-on:keyup="sendmymessage" />
              </div>
              <div class="flex-shrink">
                <font-awesome-icon class="mr-1 h-4 text-gray-400 hover:text-gray-600 cursor-pointer" :icon="['fas', 'cloud-upload-alt']" v-bind:class="[files.length? 'text-gray-600': '']" @click="$refs.file_upload.click()"></font-awesome-icon>
                <!-- <form @submit.prevent="onSubmit" enctype="multipart/form-data" style="display: none;">
                    <file-field v-model="files" />
                </form>-->
                <input type="file" @change="handleFileChange" style="display:none;" ref="file_upload" />
        ...
        <div class="mt-10 lg:mt-0 lg:w-2/6 bg-white shadow-md">
          ...
          <div class="flex justify-between items-center border-b border-solid border-gray-300 p-5">
            <div class="grow">
              <div class="flex">
                <div class="mr-5">
                  <img class="w-16 rounded-full" :src="selectedAvarta" />
                </div>
                <div>
          ...
          <div class="border-b border-solid border-gray-300 p-5">
            <div class="cursor-pointer flex justify-between items-center border-solid border-gray-300">
              <div class="grow">
                <h1 class="text-lg font-semibold text-gray-600">Options</h1>
              </div>
              <div class="flex-shrink">
                <font-awesome-icon class="mr-1 h-4 text-gray-500 hover:text-gray-600 cursor-pointer" :icon="['fas', options.opt1? 'chevron-down':'chevron-right' ]" @click="click_option('opt1')" />
              </div>
            </div>
            <div class="flex mt-3 cursor-pointer items-center" :class="[ options.opt1?'': 'hidden']">
              <div class="mr-3">
                <font-awesome-icon class="mr-1 h-4 text-gray-400 hover:text-gray-500" :icon="['fas', 'file']" />
              </div>
              <div>
          ...
          <div class="border-b border-solid border-gray-300 p-5">
            <div class="cursor-pointer flex justify-between items-center">
              ...
              <div class="flex-shrink">
                <font-awesome-icon class="mr-1 h-4 text-gray-500 hover:text-gray-600 cursor-pointer" :icon="['fas', options.opt2? 'chevron-down':'chevron-right']" @click="click_option('opt2');$forceUpdate()" />
              </div>
            </div>
            <div class="flex mt-3 cursor-pointer items-start shared-list" :class="[ options.opt2?'': 'hidden']">
              <div v-for="(file ,index) in added_file" :key="`${index}`" class="flex cursor-pointer items-center">
                ...
                <div>
                  <a :href="file.url" download target="_blank">
                    <h1 class="font-semibold text-ideeza">{{file.name}}</h1>
                  </a>
                </div>
              </div>
            </div>
          ...
          <div class="p-5">
            <div class="cursor-pointer flex justify-between items-center">
              ...
              <div class="flex-shrink">
                <font-awesome-icon class="mr-1 h-4 text-gray-500 hover:text-gray-600 cursor-pointer" :icon="['fas', options.opt3? 'chevron-down':'chevron-right']" @click="click_option('opt3')" />
              </div>
            </div>
...
<script>
import LeftMenu from "~/components/service-provider/messages/left-side-menu.vue";
