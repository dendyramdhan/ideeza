      ...
      <div class="main-contents p-5">
        <simple-table :searchbox="false" :add="false" :header="true" title="Projects" border="border-gray">
          ...
          <template v-slot:th>
            ...
            <th class="border-t border-b border-blue-300 w-13 text-ideeza-dark p-3">

            </th>
          </template>
          <tr class="flex w-full mb-4" v-for="(project,index) in projects" @click="$router.push('/technician/tracking/'+project.id)">
            <td class="w-1/5 text-ideeza-dark font-semibold" :class="{'border-b':projects.length-1 != index}">
              <div class="flex">
            ...
            <td class="w-2/12 text-sm text-ideeza-dark font-semibold" :class="{'border-b':projects.length-1 != index}">{{project.name}}</td>
            <td class="w-2/12 text-sm text-ideeza-dark text-center" :class="{'border-b':projects.length-1 != index}">
                <img v-for="image in project.service_providers" :src="image.url" class="avatar">
                +{{project.service_providers.length}}
            </td>
            <td class="w-2/12 text-sm text-ideeza-dark font-semibold pl-2 text-left" :class="{'border-b':projects.length-1 != index}"><div class="ml-2">{{project.deadline}}</div></td>
            <td class="w-13 text-sm text-ideeza-dark font-semibold pl-2 text-left" :class="{'border-b':projects.length-1 != index}">
                <div class="ml-5">{{project.completed}}%</div></td>
            <td class="w-2/12 text-sm text-ideeza-dark font-semibold pl-2 text-left" :class="{'border-b':projects.length-1 != index}">
